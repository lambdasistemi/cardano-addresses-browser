...
const scriptToJson = (script) => {
  switch (script.kind) {
    ...
    case "All":
      return { all: script.scripts.map(scriptToJson) };
    case "Any":
      return { any: script.scripts.map(scriptToJson) };
    case "AtLeast":
      return {
        some: {
          at_least: script.required,
          from: script.scripts.map(scriptToJson),
        },
      };
    case "ActiveFrom":
